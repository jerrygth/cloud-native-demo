    ...
    const mutation = useMutation({
        mutationFn: async ({ userName, userEmail, ...orderRequest }) => {
            ...
            if (!scriptLoaded || !window.Razorpay) {
                throw { error: "script_error", message: "Could not load Razorpay checkout. Please try again.", status: 0 };
            }
            ...
            return new Promise((resolve, reject) => {
                const options = {
                    ...
                    // ── SUCCESS HANDLER ──────────────────────────────────────
                    // Razorpay calls this after successful payment in the popup.
                    // These 3 values MUST be verified on the server — never trust them blindly.
...
export const QUERY_KEYS = {
    paymentHistory: (page) => ["payments", "history", page],
};
...
export function useRefundMutation() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (request) => paymentApi.refund(request),
        onSuccess: () => {
...
const LOGIN_URL = import.meta.env.VITE_LOGIN_URL || "http://localhost:8081/oauth2/authorization/auth0";
const LOGOUT_URL = import.meta.env.VITE_LOGOUT_URL || "http://localhost:8081/logout";
